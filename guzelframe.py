...
class BotFramework(OutputChannel):
    ...
    async def get_user_information(self) -> Optional[Text]:
        email = ""
        get_user_uri = f'{self.global_uri}conversations/{self.conversation["id"]}/members'
        headers = await self._get_headers()
        ...
        if get_user.ok:
            user_response = json.loads(get_user.text)
...
class BotFrameworkInput(InputChannel):
    ...
    def blueprint(
        ...
        async def webhook(request: Request) -> HTTPResponse:
            ...
            conversation_id = postdata.get("conversation").get("id")
            ...
            try:
                if postdata["type"] == "message":
                    ...
                    mail  = (await out_channel.get_user_information())
                    def mail_obje(mail):
                        a_key = "email"
                        values_of_key = ([a_dict[a_key] for a_dict in mail])
                        return values_of_key
                    xx=str(mail_obje(mail))   
                    def karakter_donus(metin):
                    ...
                    user_msg = UserMessage(
                        text=karakter_donus(postdata.get("text", "")),
                        output_channel=out_channel,
                        sender_id=postdata["from"]["name"] + " " + xx.strip("[]"),
                        #xx=json.dumps(mail),
                    ...
                    """with sqlite3.connect('deneme.db') as vt:
                            im = vt.cursor()
                            im.execute("insert into user_info values (?)",user_msg())
